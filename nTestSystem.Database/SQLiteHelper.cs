...
{
	public class SQLiteHelper: BaseHelper
    {
        SqliteConnection scn;
        ...
        void Open()
        {
            if (scn.State != ConnectionState.Open)
            {
                if (scn
                    == null)
                {
                    scn = new SqliteConnection(ConnString);
                }
                if (scn.State.Equals(ConnectionState.Closed))
                {
        ...
        public override int ExecuteNonQuery(string sql, CommandType type, object[] para)
        {
            ...
            {
                int result = 0;
                using (var cmd = new SqliteCommand() { Connection = scn, CommandText = sql, CommandType = type })
                {
                    if (para != null)
                    {
                        foreach (SqliteParameter item in para)
                        {
        ...
        public override DataTable ExecuteTable(string sql, CommandType type, object[] para)
        {
            try
            {
                ...
                using (var cmd = new SqliteCommand())
                {
                    ...
                    if (para != null)
                    {
                        var cmdParms = (SqliteParameter[])para;
